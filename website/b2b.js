    ...
    function drawEq() {
      ...
      EQUIPMENT.filter(function (e) { return active === "All" || e.sport === active; }).forEach(function (eq) {
        ...
        function upd() {
          ...
          var t = tierFor(q);
          tierP.textContent = t.label + " → " + money(eq.price * t.mult) + "/pc";
          totalS.textContent = money(eq.price * t.mult * q);
        }
        qtyI.addEventListener("input", upd);
        $(".eq-add", card).addEventListener("click", function () {
          var q = Math.max(1, parseInt(qtyI.value, 10) || 1);
          addToCart({ title: eq.name, detail: eq.sport, unit: eq.price, qty: q });
        });
        upd();
